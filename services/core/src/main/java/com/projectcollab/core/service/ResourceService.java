...
public class ResourceService {
    ...
    public ResourceService(ManagedResourceRepository resources, AuthorizationService authorization, CryptoService crypto) {
        this.resources = resources; this.authorization = authorization; this.crypto = crypto;
    }
    ...
    private void apply(ManagedResource r, UserAccount actor, ResourceInput i) {
        ...
        if (i.account() != null) r.accountCiphertext = crypto.encrypt(i.account());
        if (i.secret() != null) r.secretCiphertext = crypto.encrypt(i.secret());
        if (i.token() != null) r.tokenCiphertext = crypto.encrypt(i.token());
